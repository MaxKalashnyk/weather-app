...
export default class FavouriteLocations extends Component {
  ...
  weatherCurrentUpdate(weatherCurrent) {
    ...
    if (weatherCurrent.weatherData.placeId) {
      AppState.update('ISFAVOURITEPLACE', { isFavourite: matchedItem });
    }
  }
  ...
  handleFavouritePlace(place) {
    ...
    putItemToLocalStorage('favouritePlaces', this.state.storageFavouritePlaces);
  }
        ...
        children: [
          {
            ...
            children: [
              {
                tag: 'h3',
                classList: ['user-activity-title', 'user-activity-title-fav'],
                content: 'favourite'
              },
              {
                tag: 'button',
                classList: ['remove-button', 'remove-button-favourite'],
                eventHandlers: {
            ...
            children: [
              {
                ...
                children: this.state.storageFavouritePlaces.map(placeItem => {
                  return {
                    tag: 'li',
                    classList: ['user-activity-list-item'],
                    content: placeItem.formattedPlace || placeItem.name,
                    attributes: [
                      {
                        name: 'data-name',
                        value: placeItem.place || placeItem.name
                      },
                      {
                        name: 'data-placeid',
